...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("A simple exclusive wake up example");

#define DEV_NAME "simple_block_2_dev"

spinlock_t my_lock;
static long my_data = 0;
...
static long simple_block_2_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd){
        case WQ:
            printk("simple_block_2: Process %d (%s) sleep\n", current->pid, current->comm);
    ...
    printk("simple_block_2: Process %d (%s) wake up to reduce my data %ld\n", current->pid, current->comm, my_data);
    spin_lock(&my_lock);
    my_data--;
    spin_unlock(&my_lock);
...
static int simple_block_2_open(struct inode *inode, struct file *file) { return 0; }
static int simple_block_2_release(struct inode *inode, struct file *file) { return 0; }
