...
static long simple_block_1_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    spin_lock(&my_lock);
    printk("simple_block_1: Process %d (%s) wake up to reduce my data %ld\n", current->pid, current->comm, my_data);
    my_data--; 
    spin_unlock(&my_lock);
...
static int simple_block_1_open(struct inode *inode, struct file *file) { return 0; }
static int simple_block_1_release(struct inode *inode, struct file *file) { return 0; }
