...
#include <linux/slab.h> // kmalloc, kfree를 위해 보통 추가합니다 (이미지에는 없지만 추가를 권장합니다)
...
static int __init simple_workqueue_init(void) {
    ...
    ret = request_irq(irq_num, simple_workqueue_isr, IRQF_TRIGGER_FALLING,
                        "switch_irq", NULL);
    
    if(ret) {
        printk("Unable to rqeust IRQ: %d\n", ret);
