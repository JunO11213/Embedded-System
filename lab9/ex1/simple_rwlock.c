...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("A simple example for rwlock");
...
static long simple_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
    ...
    switch(cmd){
        ...
        case IOCTL_READ:
            ...
            if(my_data != NULL){
                old = my_data;
                printk("simple_rwlock : [R] Read my_data = %ld\n", *old);
                mdelay(500);
                printk("simple_rwlock : [R] After delay, Read my_data = %ld\n", *old);
            } else {
                printk("simple_rwlock : [R] my_data is NULL\n");
            }
        ...
        case IOCTL_WRITE:
            ...
            printk("simple_rwlock : [W] write new data = %ld\n", arg);
